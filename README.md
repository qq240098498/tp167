...
7. 城市归属全系统同一套口径：收件城市先按分区登记的别名归到正式城市，再按正式城市定分区（运单页、单条计费、出账走的都是这一套）。认不出归属的城市按「未归属」处理：不能单条计费，出账时会被挡住并把城市列出来，不会默算到任何一个分区
8. 出账时同一客户同一账期的运单按分区分组，组内合起来算一次首重续重，再按各自的计费重量分摊
